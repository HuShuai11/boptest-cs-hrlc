"""【消融 L3】T_rule + TCN_T(ch=32/64) + TCN_U full (rmax=0.25, ff=0.03) → AdpPID+FF."""
...
from method.rule_teacher import paper_comfort_bounds, paper_trule, compute_future_features, make_tcn_features, high_level_teacher
...
FROZEN_TCNU = dict(lr=0.0001, ch0=16, ch1=32, rmax=0.25, ff=0.03, uw=288, ur=288, ti=4)
...
def run(scenario="Typical", days=14, warmup_hours=168):
    ...
    rid = f"ablation_L3_{scenario}_{ts}"
    _ABL = os.path.dirname(_HERE)

    print(f"[Init] Ablation L3: TCN_T+TCN_U full (rmax=0.25, ff=0.03) | seed={SEED}", flush=True)
    ...
    for step in range(total_steps):
        ...
        t_low, t_high = paper_comfort_bounds(dt_now); occ = is_occupied(dt_now) if 'is_occupied' in dir() else (not (dt_now.weekday() < 5 and 7.0 <= dt_now.hour + dt_now.minute / 60.0 < 20.0))
        ...
        with torch.no_grad():
            ...
            rd = float(r_vec[0, 2].item()); u_ff_nn = float(u_ff_vec[0, 0].item())
        ...
        alpha_u_val = alpha_u_sched(step, FROZEN_TCNU["uw"], FROZEN_TCNU["ur"], 1.0)
        margin_l = tz - t_low; margin_h = t_high - tz; action_gate = 1.0
        if margin_l < 0.6 and u_ff_nn < 0: u_ff_nn = 0.0
        if margin_h < 0.6 and u_ff_nn > 0: u_ff_nn = 0.0
        if margin_l < 0.3 or margin_h < 0.3: action_gate = 0.0
        if u_pid <= 0.02 and u_ff_nn < 0: u_ff_nn = 0.0
        if str(scenario).lower() == "typical" and tz > 23.5 and u_pid <= 0.02: action_gate = 0.0
        ...
        if (step + 1) % SPD == 0:
            ...
            print(f"  Day {(step + 1) // SPD}/{days} Tz={tz_n:.2f} Trule={T_rule:.2f} "
                  f"Kp={kp_eff:.3f} FF={u_ff_nn:.4f} Tset={T_set:.2f} u={u_final:.3f} "
    ...
    kd = {"ablation": "L3", "level": 3, "method": "TCN_T + TCN_U full → AdpPID+FF",
          "scenario": scenario, "seed": SEED,
          "tcn_ch0": FROZEN_TCN["tcn_ch0"], "tcn_ch1": FROZEN_TCN["tcn_ch1"],
          "lr": FROZEN_TCN["lr"], "tcnu_rmax": FROZEN_TCNU["rmax"],
          "tcnu_ff_scale": FROZEN_TCNU["ff"],
    ...
    print(f"[KPI] L3 {scenario}: tdis={kd['tdis']:.4f} cost={kd['cost']:.6f}")
    return kd


if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Ablation L3: TCN_T + TCN_U full")
